        ...
        whatsapp: {
            ...
            send: function (to, message = '', attachments = [], phone_id = false, onSuccess = false) {
                SBF.ajax({
                    ...
                    if (response.error) dialog(response.error.message);
                    if (onSuccess) onSuccess(response);
                });
            },
                    ...
                    if (response.length) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                let conversation_code = this.getListCode(item, status == 0 || status == 2 ? 'new' : null);
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : (source == 'wa' ? 'WhatsApp' : (source == 'tm' ? 'Text message' : (source == 'ig' ? 'Instagram' : (source == 'tg' ? 'Telegram' : (source == 'tk' ? 'Tickets' : (source == 'wc' ? 'WeChat' : (source == 'em' ? 'Email' : (source == 'tw' ? 'Twitter' : (source == 'bm' ? 'Business Messages' : (source == 'vb' ? 'Viber' : source)))))))))), sb_('Source'));
                }
            }
        ...
        updateRequiredFields: function (user_type) {
            ...
            profile_edit_box.find('#email input').prop('required', agent || user_type == 'user');
        }
    }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'armember': 'ARMember', 'telegram': 'Telegram', 'viber': 'VIber', 'wechat': 'WeChat', 'twitter': 'Twitter', 'zendesk': 'Zendesk', 'gbm': 'Google Business Messages', 'martfury': 'Martfury' };
                let updates = false;
        ...
        */
        ...
        $(settings_area).on('click', '#whatsapp-twilio-btn .sb-btn, #sms-btn .sb-btn, #wechat-btn .sb-btn, #twitter-callback .sb-btn, #gbm-webhook .sb-btn, #viber-webhook .sb-btn', function (e) {
            let id = $(this).closest('[id]').attr('id');
            dialog(`<pre>${SB_URL + (id == 'sms-btn' ? '/include/api.php' : ('/apps/' + (id == 'wechat-btn' ? 'wechat' : (id == 'twitter-callback' ? 'twitter' : (id == 'gbm-webhook' ? 'gbm' : (id == 'viber-webhook' ? 'viber' : 'whatsapp')))) + '/post.php')) + cloudURL().replace('&', '?')}</pre>`, 'info');
            return false;
            e.preventDefault();
        });
