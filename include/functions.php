...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger', 'whatsapp', 'armember', 'viber', 'telegram', 'wechat', 'twitter', 'zendesk', 'gbm', 'martfury'];
...
function sb_get_active_user($login_data = false, $database = false, $login_app = false, $user_token = false) {
    ...
    if ($login_app !== false) {
        ...
        } else if ($app == 'default' && (!$return || sb_isset($return, 'email') != sb_isset($login_app_data, 'email'))) {
...
function sb_js_admin() {
    ...
    if ($cloud) {
        ...
        $settings['cloud'] = ['email' => $cookie_cloud['email'], 'id' => sb_db_get('SELECT id FROM sb_users WHERE email = "' . $cookie_cloud['email'] . '"')['id'], 'cloud_user_id' => $cookie_cloud['user_id'], 'token' => $cookie_cloud['token']];
    }
...
function sb_direct_message($user_ids, $message, $subject = false) {
    $sources = ['whatsapp' => 'wa', 'messenger' => 'fb', 'telegram' => 'tg', 'viber' => 'vb', 'twitter' => 'tw', 'instagram' => 'ig', 'wechat' => 'wc', 'google' => 'bm', 'tickets' => 'tk'];
...
function sb_check_conversations_assignment($conversations_ids, $agent_id = false, $department = false) {
    ...
    $conversations_ids = sb_db_get('SELECT id FROM sb_conversations WHERE id IN (' . sb_db_escape(implode(',', $conversations_ids)) . ') AND ' . ($agent_id ? ('agent_id <> ' . sb_db_escape($agent_id, true)) : '') . ($agent_id && $department ? ' AND ' : '') . ($department ? ('department <> ' . sb_db_escape($department)) : ''), false);
...
function sb_update_messages_status($conversation_id, $message_ids, $user_id = false) {
...
function sb_routing_and_department_db($table_name = 'sb_conversations', $users = false) {
    ...
    return $query ? ($users ? ' AND id IN (SELECT user_id FROM sb_conversations WHERE ' . substr($query, 4) . ')' : $query) : '';
}
...
function sb_send_message($sender_id, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false, $recipient_id = false) {
    ...
    if ($sender_id != -1) {
        ...
        if (!$conversation) return trigger_error('Conversation not found.');
        ...
        if ($security || $user_id == sb_get_active_user_ID() || !empty($GLOBALS['SB_FORCE_ADMIN'])) {
            ...
            if ((!$queue_active || (!$is_sender_agent && !sb_agents_online())) && !$is_sender_bot && !$is_human_takeover) {
                ...
                if (($agent_check && sb_get_setting('notify-agent-email')) || (!empty($user['email']) && (($user_check && sb_get_setting('notify-user-email')) || sb_isset($conversation, 'source') == 'em'))) {
            ...
            sb_webhooks('SBMessageSent', ['user_id' => $sender_id, 'conversation_user_id' => $user_id, 'conversation_id' => $conversation_id, 'conversation_status_code' => $conversation_status_code, 'message_id' => $response, 'message' => $message,' attachments' => $attachments]);
    ...
    if (in_array($name, ['registration', 'registration-tickets', 'login', 'login-tickets', 'timetable', 'articles', 'woocommerce-cart'])) {
        ...
        switch ($name) {
            ...
            case 'registration':
                ...
                $user = $active_user && !sb_is_agent($active_user['user_type']) ? sb_get_user($active_user['id'], true) : ['profile_image' => '', 'first_name' => '', 'last_name' => '', 'email' => '', 'password' => '', 'user_type' => 'visitor', 'details' => []];
                $visitor = !$user || $user['user_type'] == 'visitor' || $user['user_type'] == 'lead';
    ...
    if (isset($setting)) {
        ...
        switch ($type) {
            ...
            case 'select':
                ...
                $content .= '<select>';
                for ($i = 0; $i < count($values); $i++) {
                    $content .= '<option value="' . $values[$i][0] . '">' . sb_s($values[$i][1]) . '</option>';
                }
                $content .= '</select>';
                break;
            case 'checkbox':
            ...
            case 'upload-image':
                ...
                break;
            case 'input-button':
                $content .= '<input type="text"><a class="sb-btn">' . sb_s($setting['button-text']) . '</a>';
                break;
            case 'button':
                $content .= '<a class="sb-btn" target="_blank" href="' . $setting['button-url'] . '">' . sb_s($setting['button-text']) . '</a>';
                break;
            case 'multi-input':
                $values = $setting['value'];
                for ($i = 0; $i < count($values); $i++) {
                    $sub_type = $values[$i]['type'];
                    $content .= '<div id="' . $values[$i]['id'] . '" data-type="' . $sub_type . '" class="multi-input-' . $sub_type . '"><label>' . sb_s($values[$i]['title']) . '</label>';
                    switch ($sub_type) {
                        case 'text':
                            $content .= '<input type="text">';
                            break;
                        ...
                        case 'select':
                            ...
                            break;
                        case 'button':
                            $content .= '<a class="sb-btn" target="_blank" href="' . $values[$i]['button-url'] . '">' . sb_s($values[$i]['button-text']) . '</a>';
                            break;
                        case 'select-checkbox':
                            $items = $values[$i]['value'];
                            $content .= '<input type="text" class="sb-select-checkbox-input" readonly><div class="sb-select-checkbox">';
                            for ($i = 0; $i < count($items); $i++) {
                                $content .= '<div class="multi-input-checkbox"><input id="' . $items[$i][0] . '" type="checkbox"><label>' . sb_s($items[$i][1]) . '</label></div>';
                            }
                            $content .= '</div>';
                            break;
                    }
            ...
            case 'repeater':
                ...
                for ($i = 0; $i < count($setting['items']); $i++) {
                    ...
                    $content .= '<div>' . (isset($item['name']) ? '<label>' . sb_s($item['name']) . '</label>' : '');
                    switch ($item['type']) {
                        case 'text':
                        case 'number':
                        case 'password':
            ...
            case 'select-checkbox':
                ...
                for ($i = 0; $i < count($values); $i++) {
                    $content .= '<div id="' . $values[$i]['id'] . '" data-type="checkbox" class="multi-input-checkbox"><input type="checkbox"><label>' . sb_s($values[$i]['title']) . '</label></div>';
                }
                $content .= '</div>';
                break;
        }
...
function sb_download_file($url, $file_name = false, $mime = false, $header = [], $recursion = 0) {
    ...
    if ($extension && !in_array($extension, $allowed_extensions)) return 'extension-not-allowed';
    if ($file_name && !sb_is_error($url) && !empty($url)) {
        ...
        if (!file_exists($path . '/' . $file_name) && $recursion < 3) {
...
function sb_automations_is_sent($active_user_id, $automation, $repeat_id = false) {
    $history = sb_get_external_setting('automations-history', []);
    if ($active_user_id) {
